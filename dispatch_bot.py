...
_RE_DOPY_GENERIC = re.compile(
    r"(?:(?<![а-яіїєґ])доп(?:и|ы|ов)?(?![а-яіїєґ])[ \t]*(\d+(?:[.,]\d+)?))"
    r"|(?:(\d+(?:[.,]\d+)?)[ \t]*(?<![а-яіїєґ])доп(?:и|ы|ов)?(?![а-яіїєґ]))",
    re.IGNORECASE,
)
