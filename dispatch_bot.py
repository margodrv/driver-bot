...
_LOADER_TEXT_RE = re.compile(
    # "експедитор"/"экспедитор" - подтверждено логистом-владельцем
    # (03.09, Алексей Конюхов, "Тар экспедитор 700/350"): в этой системе
    # экспедитор трактуется как та же роль, что и грузчики - свой тариф
    # (база/доп.час), показывается той же строкой "Грузчики:" в превью.
    #
    # "(?<!без)(?<!без[ \t])" перед каждым вариантом - реальный кейс
    # 09.09 (Анастасія, Притыченко): "3Т с ГБ БЕЗ грузчиков" - слово
    # "грузчиков" физически присутствует в тексте, но с явным отрицанием
    # ПЕРЕД ним - грузчиков в заказе НЕТ, ровно наоборот тому, что раньше
    # думал код (голый substring-поиск не различает "упомянуты" и
    # "явно отрицаются"). Без этой проверки `_apply_generic_combined_avto_gruzchiki`
    # считал, что грузчики есть, и ошибочно склеивал "Авто+грузчики:
    # 2900 (2ч)/800" вместо простого "Авто: 2900 (2ч)/800". Два
    # отдельных lookbehind фиксированной ширины (без пробела - на случай
    # слитного "безвантажників", и с одним пробелом - обычный случай) -
    # Python re не поддерживает lookbehind переменной длины.
    r"(?<!без)(?<!без[ \t])(?<![а-яіїєґ])вантаж\w*"
    r"|(?<!без)(?<!без[ \t])(?<![а-яіїєґ])груз\w*"
    r"|(?<!без)(?<!без[ \t])(?<![а-яіїєґ])вант(?![а-яіїєґ])"
    r"|(?<!без)(?<!без[ \t])(?<![а-яіїєґ])експедитор\w*"
    r"|(?<!без)(?<!без[ \t])(?<![а-яіїєґ])экспедитор\w*",
    re.IGNORECASE,
)
...
# Второй, отдельный однозначный маркер доп.точки (реальный кейс 03.09,
# Игорь/Тимур): "доп точка 600 грн" - буквально слово "доп" ПЕРЕД
# "точка", число ПОСЛЕ (не глифом к "пром", другой порядок слов). В
# отличие от голого "число ... точка" (ловушка, см. комментарий выше -
# "5500/5000 точка 500грн" ложно ловило чужое 5000) - тут ОБЯЗАТЕЛЕН
# литерал "доп" прямо перед "точка" И "грн" сразу после числа, двойное
# требование резко снижает риск случайного совпадения с посторонним
# числом рядом со словом "точка" по другой причине.
...
def _apply_keyword_overrides(result: dict, order_text: str):
    ...
    if not result.get("_dop_tochka_trusted"):
        m = _RE_DOP_TOCHKA_GENERIC.search(order_text or "") or _RE_DOP_TOCHKA_EXPLICIT.search(order_text or "")
        if m:
...
_RE_LOADER_COUNT_PLUS = re.compile(r"(\d+)[ \t]*(?:вантажник\w*|грузчик\w*|експедитор\w*|экспедитор\w*)", re.IGNORECASE)
...
_RE_SEPARATE_LOADERS_LINE = re.compile(r"(?:вантажник\w*|грузчик\w*|експедитор\w*|экспедитор\w*)[:\s]*\d+(?:[.,]\d+)?", re.IGNORECASE)
...
# "(?:грн[ \t]*)?" перед "/час" - реальный кейс 03.09: "Грузчики
# 400грн/час" - между ставкой и "/час" вклинилась единица "грн" (тот же
# класс бага, что уже чинили несколько раз - раздел 9 п.2). Без этого
# regex вообще не матчился, и голая ставка "400" ошибочно подхватывалась
# позже, более слабым фолбэком `_RE_BARE_GRUZ_LINE` - без удвоения (база
# оставалась 400 вместо правильных 800) и без доп.часа вообще.
#
# "(?:ч[а-яё]*|год\w*)" вместо голого "час" - реальный кейс 09.09 (Яна):
# "Вантажники: 300грн/год" - этот диспетчер пишет "/год" там, где другие
# пишут "/час" (та же путаница "год" как обозначение часа, что уже
# чинили в `_RE_TAR_HOURS_SKIP`, раздел 0 от 02.09). Подтверждено
# логистом-владельцем: стандарт тот же, что и для "/час" - база = ставка
# ×2 (минимальный заказ грузчиков - 2 часа), доп.час = ставка.
#
# "[ \t]*:?[ \t]*" после слова-роли (не просто "[ \t]*") - тот же
# реальный кейс 09.09: "Вантажники: 300грн/год" - двоеточие сразу после
# "Вантажники" не пропускалось вообще, регэксп не матчился НИ ОДНИМ
# способом, и ставка подхватывалась более слабым фолбэком
# `_RE_BARE_GRUZ_LINE` (который двоеточие как раз пропускает) - без
# удвоения и без доп.часа, та же картина, что и в фиксе 03.09 для "грн"
# между числом и "/час".
_RE_LOADER_SINGLE_RATE = re.compile(
    r"(?:\b1[ \t]*)?(?:вантажник\w*|грузчик\w*|експедитор\w*|экспедитор\w*)[ \t]*:?[ \t]*(\d+(?:[.,]\d+)?)[ \t]*(?:грн[ \t]*)?/[ \t]*(?:ч[а-яё]*|год\w*)", re.IGNORECASE
)
...
def _split_gruzchiki_dopy_by_plausibility(result: dict):
    ...
    ВАЖНО (реальный кейс 03.09, Алексей Конюхов - "+400/год +20 доп.
    вантажники"): GPT иногда кладёт число ОДНОВРЕМЕННО в gruzchiki_dopy
    (как "лишнее число рядом с грузчиками") И в своё собственное
    'неponyatno' - но САМО ЖЕ число уже могло быть классифицировано чуть
    раньше по цепочке явным словесным триггером (тут - "20 доп." через
    `_RE_DOPY_GENERIC` в `_apply_keyword_overrides`, этажи+проходы уже
    =20). `_reconcile_neponyatno` (раньше этой функции по цепочке)
    убирает дубль-предупреждение по значению - но ЭТА функция о нём не
    знала и, видя etazh_prohod_assigned=True (уже занято ДРУГИМ, но на
    самом деле ТЕМ ЖЕ САМЫМ числом), снова добавляла "не понял" по нему
    же, воскрешая только что убранный дубль. Пропускаем такие числа
    молча (они и так уже видны в превью под своим настоящим названием).
    """
    ...
    for key in ("dop_tochka", "gidrobort"):
        d = tj.get(key)
        if d and d.get("summa") is not None:
            already_classified.add(d["summa"])
    for key in ("km_stavka", "etazhi_stavka", "prohody_stavka"):
        v = tj.get(key)
        if v is not None:
            already_classified.add(v)
    ves = tj.get("ves")
    if ves and ves.get("stavka") is not None:
        already_classified.add(ves["stavka"])
    for dop in tj.get("prochie_dopy") or []:
        if dop.get("summa") is not None:
            already_classified.add(dop["summa"])

    remaining = []
    neponyatno = list(result.get("neponyatno") or [])
    etazh_prohod_assigned = tj.get("etazhi_stavka") is not None or tj.get("prohody_stavka") is not None
    for val in dopy:
        if val in already_classified:
            continue  # то же самое число уже классифицировано в другое поле выше по цепочке - не дублируем
...
# "год...", вся группа целиком считается отсутствующей - число НЕ
# "откусывается" от настоящей базы (см. раздел 9 п.7/9 документации).
#
# "год\w*" (не "годин\w*") - реальный кейс 02.09: "Тар авто 2год
# 7520грн/650грн" - сокращение "год" (без "ин") тоже реально
# встречается, а старый паттерн требовал буквально "годин" (минимум 5
# букв), из-за чего пропуск часов не срабатывал вообще, и "2" (от
# "2год") ошибочно захватывалось как САМА БАЗА авто вместо настоящих
# 7520 - т.к. эта проверка объявлена "финальной, самой авторитетной" и
# переопределяет даже верно разобранное GPT значение (см. раздел 9 п.7 -
# тот же класс бага, что уже дважды чинили 24.08 для короткой/полной
# формы часов). "годин\w*" был частным случаем "год\w*" (сама "годин"
# начинается с "год"), так что более широкий паттерн не сужает уже
# работавшие случаи.
...
_RE_TAR_AVTO_NUMBERS = re.compile(
    # "[ \t]*:?[ \t]*" после "авто" (не просто "[ \t]*") - реальный кейс
    # 03.09 (Тимур/Игорь): "Тариф \nАвто: 6000/1650 БН" - между "Авто" и
    # числом стоит двоеточие, которое старый паттерн не пропускал вообще
    # (только пробел/таб) - регэксп не матчился НИ ОДНИМ из двух
    # запасных вариантов (`_RE_TAR_NUMBERS` тоже не пропускает слово
    # "авто" само по себе), и строка "Авто:" целиком терялась из превью,
    # даже без единого "не понял". Двоеточие сделано опциональным именно
    # в пределах строки (пробел/таб, БЕЗ `\s`/переноса) - не хотим, чтобы
    # поиск "перепрыгивал" через перенос строки в поисках первого
    # попавшегося числа где-то дальше, если рядом с "авто" числа нет
    # вообще (см. раздел 9 п.7 - на подобных "первое число после
    # опорного слова" граблях уже спотыкались).
...
_RE_TAR_GRUZ_NUMBERS = re.compile(
    r"тар(?:иф)?[:\s]*\n?[ \t]*(?:вантажник\w*|груз\w*|(?<![а-яіїєґ])вант(?![а-яіїєґ])|експедитор\w*|экспедитор\w*)[ \t]*"
    + _RE_TAR_HOURS_SKIP +
    r"(\d+(?:[.,]\d+)?)(?:[ \t]*/[ \t]*(\d+(?:[.,]\d+)?))?",
    re.IGNORECASE,
)
...
# Точка маршрута обозначена знаком "?" вместо конкретного адреса
# ("Т2 - Т?", "Т?", "Т2?") - сигнал "будет несколько точек, точный
# адрес пока неизвестен" (см. докстринг _apply_hodka_context_to_dop_tochka,
# реальный кейс 16.09). Лукбехайнд на кириллицу перед "т" - чтобы не
# зацепить конец слова вроде "результат?" (там перед "т" стоит "а").
...
def _apply_hodka_context_to_dop_tochka(result: dict, order_text: str):
    ...
    Ещё один вариант (16.09, подтверждено логистом-владельцем явным
    правилом, реальный кейс Алексей Конюхов): точка маршрута обозначена
    знаком "?" вместо конкретного адреса - например "Т2 - Т?" (второй
    адрес ещё не определён/будет несколько адресов). Логист: "если
    около Т2 знак ? - значит будет несколько точек и значит [число] это
    точка". Тот же смысл, что и Т3/Т4 (маршрут не сводится к одной
    точке назначения) - только неопределённость выражена знаком "?" при
    номере точки, а не явным номером Т3+. Ловим "т" (с необязательной
    цифрой перед) сразу за которым идёт "?" (`_RE_ROUTE_UNKNOWN_STOP`).
    ...
    has_extra_stops = (
        ...
        or bool(re.search(r"\d+(?:[ \t]*-[ \t]*\d+)?[ \t]*(?:шкіл\w*|школ\w*|адрес\w*|об[''`]?єкт\w*)", text_low, re.IGNORECASE))
...
def build_tariff_preview(tariff: dict, author_line: str = "", edited_fields: set = frozenset()) -> str:
    # Реальный кейс (02.09): если в тексте заявки есть строка вида
    # "Логіст Яна @Yana_TK_Gorod" (это контакт логиста, который прислал
    # эту КОНКРЕТНУЮ заявку от своего контрагента - обычная подпись в
    # заявке, не адресация чего-либо боту), extract_order_author() до сих
    # пор находил её и добавлял отдельной строкой "@Yana_TK_Gorod" ПЕРЕД
    # "📋 Тариф по заказу" в превью. Подтверждено логистом-владельцем как
    # глюк, а не нужное поведение: "это глюк - так не нужно, в другой
    # группе такого нет" - в заказах без такой строки в тексте (как у
    # Алексея Конюхова) превью и так начинается сразу с "📋 Тариф по
    # заказу", без какого-либо тега - значит сама эта строка-тег вообще
    # не нужна ни в каком виде. `author_line`/`_order_author_line`/
    # `extract_order_author` намеренно оставлены в коде (могут ещё
    # использоваться где-то ещё, см. раздел 0/9 техсводки) - убрано
    # только само добавление этой строки в текст превью.
    lines = []
