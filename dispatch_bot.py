...
def _normalize_neponyatno_candidates(result: dict, order_text: str = ""):
    ...
    ВООБЩЕ, если в тексте заказа нет ни единого упоминания
    "гб"/"гідроборт" - логист прямо указал "ни одного упоминания про ГБ"
    - предлагать кнопку ГБ, когда ни слова про гідроборт нет вообще,
    только сбивает с толку. Симметрично уже существующей защите для
    km_stavka (_strip_bogus_km) и для dop_tochka
    ...
    has_gb_word = bool(_GIDROBORT_WORD_RE.search(order_text or ""))
    kept = []
    for item in result.get("neponyatno") or []:
        m = _NEPONYATNO_ITEM_RE.match(item)
        if not m:
